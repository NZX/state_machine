...
class MachineByDefaultTest < Test::Unit::TestCase
  ...
  def test_should_default_to_no_locking_transitions
    assert !@machine.lock_transitions?
  end
...
class MachineWithLockingTransitions < Test::Unit::TestCase
  def setup
    @machine = StateMachine::Machine.new(Class.new, :lock_transitions => true)
  end

  def test_should_set_lock_transitions
    assert @machine.lock_transitions?
  end
end
