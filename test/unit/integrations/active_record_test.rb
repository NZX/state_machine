...
module ActiveRecordTest
  ...
  if RUN_CONCURRENCY
    class MachineWithLockingTest < BaseTestCase
      ...
      def test_should_obtain_db_lock
        pid = ar_fork do
        ...
        sleep 1 # ensure the above fork gets the lock
        ...
        begin
          ...
          send_signal(pid) # no zombies pls
        end
      end

    private
      # Just a normal fork that resets AR connections so they're not shared
      # across the new processes
      def ar_fork
        pid = fork do
      ...
      def wait_for_signal
        while true
          break if File.exists?("signal.#{$$}")
        end
        FileUtils.rm("signal.#{$$}")
      end

      def send_signal(pid)
        FileUtils.touch("signal.#{pid}")
      end
    end
  end
