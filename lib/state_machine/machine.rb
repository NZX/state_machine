...
module StateMachine
  ...
  class Machine
      ...
      def find_or_create(owner_class, *args, &block)
        ...
        if owner_class.respond_to?(:state_machines) && machine = owner_class.state_machines[name]
          ...
          if machine.owner_class != owner_class && (options.any? || block_given?)
            ...
            machine.lock_transitions = options[:lock_transitions] if options.include?(:lock_transitions)
    ...
    def initialize(owner_class, *args, &block)
      options = args.last.is_a?(Hash) ? args.pop : {}
      assert_valid_keys(options, :attribute, :initial, :action, :plural, :namespace, :integration, :messages, :use_transactions, :lock_transitions)
      ...
      self.lock_transitions = options[:lock_transitions] if options.include?(:lock_transitions)
    ...
    def lock_transitions=(lock)
      @lock_transitions = lock
    end
